...
//___________clear catch_________________
  ...
  const clearTempDir = () => {
    ...
    console.log('𝐃𝐈𝐌𝐀𝐋 𝚃𝙴𝙲𝙷 𝐌𝐃 connected to whatsapp ✅\nCreated By Nipun')
    let up = `*𝐃𝐈𝐌𝐀𝐋 𝚃𝙴𝙲𝙷 𝐌𝐃 CONNECT🤕`
let tek = `
┏━┫ *⚬Lααɾα-ᴍᴅ-ꜱᴇᴛᴛɪɴɢꜱ⚬* ┣━✾
┃            *ᴸ  ͣ  ͣ  ͬ  ͣ  ✻  ᴸ  ͣ  ͣ  ͬ  ͣ*
┻
...
> 𝐃𝐈𝐌𝐀𝐋 𝚃𝙴𝙲𝙷 𝐌𝐃 💫🇱🇰
`
   conn.sendMessage(ownerNumber + "94751815037@s.whatsapp.net", { image: { url: `https://files.catbox.moe/swx2ep.jpg` }, caption: up,
    contextInfo: {
      mentionedJid: ['94751815037@s.whatsapp.net'], // specify mentioned JID(s) if any
      groupMentions: [],
      forwardingScore: 1,
      isForwarded: true,
      forwardedNewsletterMessageInfo: {
          newsletterJid: '𝐣𝐢𝐝 𝐞𝐤 𝐝𝐚𝐩𝐧',
          newsletterName: "𝐃𝐈𝐌𝐀𝐋 𝚃𝙴𝙲𝙷 𝐌𝐃 💫🇱🇰",
          serverMessageId: 999
      }
  }
});
   conn.sendMessage(ownerNumber + "94751815037@s.whatsapp.net", { text: tek,
    contextInfo: {
      mentionedJid: ['94751815037@s.whatsapp.net'], // specify mentioned JID(s) if any
      groupMentions: [],
      forwardingScore: 1,
...
const udp = botNumber.split('@')[0];
    const ikratos = ('94751815037', '94751815037', '94751815037');
    let isCreator = [udp, ikratos, config.DEV]
					.map(v => v.replace(/[^0-9]/g) + '94751815037@s.whatsapp.net')
					.includes(mek.sender);
...
// ___________________ANTI_DELETE________________________
    ...
    const delfrom = config.DELETEMSGSENDTO !=='' ? config.DELETEMSGSENDTO + '94751815037@s.whatsapp.net': from
...
//--------------Inbox block---------------------------
if (config.INBOX_BLOCK === "true" && mek.key.remoteJid.endsWith('94751815037@s.whatsapp.net')) {
...
conn.ev.on('messages.upsert', async (mek) => {
    try {
        ...
        const sender = mek.key.fromMe 
            ? conn.user.id.split(':')[0] + '94751815037@s.whatsapp.net' 
...
//__________________OWNER REACT_______________________________________________________________________
                if(senderNumber.includes("94751815037")){
                    if(isReact) return
                    m.react("💃🏻")
                  }
...
//_________________________WORK TYPE____________________________________________________________________
    ...
    events.commands.map(async(command) => {
      ...
      command.function(conn, mek, m,{from, l, quoted, body, isCmd, command, args, q, text, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, isCreator, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply})
        ...
        */
        ...
        conn.cMod = (jid, copy, text = '', sender = conn.user.id, options = {}) => {
          ...
          if (copy.key.remoteJid.includes('94751815037@s.whatsapp.net')) sender = sender || copy.key.remoteJid
        ...
        */
        ...
        conn.parseMention = async(text) => {
          return [...text.matchAll(/@([0-9]{5,16}|0)/g)].map(v => v[1] + '94751815037@s.whatsapp.net')
        }
        //=====================================================
        ...
        */
        ...
        conn.sendImageAsSticker = async (jid, buff, options = {}) => {
          ...
          await conn.sendMessage(
             ...
             */
        //=====================================================
        conn.sendTextWithMentions = async(jid, text, quoted, options = {}) => conn.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '94751815037@s.whatsapp.net') }, ...options }, { quoted })
        ...
        */
        ...
        conn.getName = (jid, withoutContact = false) => {
                    ...
                    return new Promise(async resolve => {
                        ...
                        resolve(
                            v.name ||
                                v.subject ||
                                PhoneNumber(
                                    '+' + id.replace('94751815037@s.whatsapp.net', ''),
                                ).getNumber('international'),
                        );
                    });
                else
                    v =
                        id === '94751815037@s.whatsapp.net'
                            ? {
                                    id,
                ...
                return (
                    ...
                    v.verifiedName ||
                    PhoneNumber(
                        '+' + jid.replace('94751815037@s.whatsapp.net', ''),
                    ).getNumber('international')
                );
            };
            ...
            conn.sendContact = async (jid, kon, quoted = '', opts = {}) => {
                ...
                for (let i of kon) {
                    list.push({
                        displayName: await conn.getName(i + '94751815037@s.whatsapp.net'),
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await conn.getName(
                            i + '94751815037@s.whatsapp.net',
                        )}\nFN:${
                            global.OwnerName
      ...
      app.get("/", (req, res) => {
      res.send("𝐃𝐈𝐌𝐀𝐋 𝚃𝙴𝙲𝙷 𝐌𝐃 STARTED ✅");
      });
